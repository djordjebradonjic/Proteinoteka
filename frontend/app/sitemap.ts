...
export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
  ...
  //
  // Deliberately NOT wrapped in try/catch, and NOT falling back to staticPages on a
  // zero-length result: this route revalidates every 86400s (24h), so swallowing a
  // transient backend hiccup into a "successful" empty/partial result would cache a
  // sitemap missing the entire product catalog for up to a day — the exact ISR
  // cache-poisoning shape already fixed for the homepage grid in commit 72e9b6f (see
  // project memory / that commit for the underlying Next.js behavior: throwing here makes
  // Next.js keep serving the last known-good sitemap and retry on the next request,
  ...
  for (let page = 0; page < 25; page++) {
    ...
    if (!res.ok) {
      throw new Error(`sitemap: products fetch failed on page ${page} (HTTP ${res.status})`);
    }
  ...
  if (products.length === 0) {
    throw new Error("sitemap: products fetch returned zero products, refusing to cache an empty sitemap");
  }
